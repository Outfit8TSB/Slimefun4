# Contributing to Slimefun United
This document outlines various ways how you can help contribute to Slimefun and make this a bigger and better project.<br>
All contributions must be inline with our [Code of Conduct](https://github.com/Slimefun-United/Slimefun-United/blob/master/.github/CODE_OF_CONDUCT.md) and [License](https://github.com/Slimefun-United/Slimefun-United/blob/master/LICENSE).
...
Any open Issue on our [Issues Tracker](https://github.com/Slimefun/Slimefun-United/Slimefun-United/issues) is waiting to be fixed.

This is an open-source project, and we love Pull Requests.
So if you have an idea on how to approach a known issue, feel free to make a [Pull Request](https://github.com/Slimefun-United/Slimefun-United/pulls) which fixes this bug.
You can also comment on the existing Issue, proposing your idea or communicating that you want to work on this.
...
Please visit our [Discord Server](https://discord.gg/C8M6nPyDu6) and share your ideas first, we hate to reject changes because the community disagrees.<br>
So communicating your intended changes beforehand will ensure that you don't put too much work into something that might get rejected.
...
Language Moderators are responsible for proofreading any new translations for their designated language and correct it when they see a mistake.
...
To prevent any accidents from happening, please contact us on our [Discord Server](https://discord.gg/C8M6nPyDu6) before-hand and state your intended changes.
...
3. Methods and parameters should be annotated with `@Nullable` or `@Nonnull` to indicate whether null values are accepted.
...
Try to stay inline with the code that surrounds you, having an entire package or even a single file that's filled with plenty of different and inconsistent code styles is just hard to read or maintain. That's why we want to make sure everyone follows these principles.

We provide an automated formatting check system. Please run `mvn install` to initialize it.
This project includes an `.editorconfig` file to enforce code style. If you use your own coding style, please switch to the repository’s style configuration before contributing.
...
#### 7. Style preferences
We use Spotless as our code formatting tool. Before submitting your code, you **must** run `mvn spotless:check spotless:apply` to automatically format it. Otherwise, the formatting checker will block your PR.

* Use **Spaces**, not Tabs! This project uses 4-space indentation.
* Do not excessively shorten code. Removing spaces will not make Slimefun run any faster.
...
* Try to keep so-called "guard blocks" to a minimum. One guard block is fine but having multiple guard blocks before getting to the actual code... Well, you might want to refactor your code there. Example:
```java
// guard block
if (something) {
  return;
...
* We recommend using horizontal whitespaces like this:
    ...
    * In between array initializers: `int[] array = { 1, 2, 3 };`
...
This project **requires** the use of [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) for all commit messages.

> In short, your commit message must include the following:
>
...
If it is a fix, declare it in the main commit message—no need to repeat it elsewhere.

The supported commit type prefixes match the following regex:  
`(feat(ure)?|fix|docs|style|refactor|ci|chore|perf|build|test|revert|trans)`

Additionally, if the commit is related to translation, the type should be `trans`.